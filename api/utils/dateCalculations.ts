 ...
 */
export function getLastFriday(year: number, month: number): Date {
 ...
 */
export function getCurrentPeriodDates(): { start: Date; end: Date } {
  const now = new Date();
 ...
 */
export function getPreviousPeriodDates(): { start: Date; end: Date } {
  const now = new Date();
  const thisMonth = now.getMonth();
  const thisYear = now.getFullYear();
  const prevMonth = thisMonth === 0 ? 11 : thisMonth - 1;
 ...
 */
export function getCurrentMonthlyPeriodDates(): { start: Date; end: Date } {
  const now = new Date();
  const thisMonth = now.getMonth();
  const thisYear = now.getFullYear();
 ...
 */
export function getPreviousMonthlyPeriodDates(): { start: Date; end: Date } {
  const now = new Date();
  const thisMonth = now.getMonth();
  const thisYear = now.getFullYear();
  const prevMonth = thisMonth === 0 ? 11 : thisMonth - 1;
