...
import { getCurrentPeriodDates, getPreviousPeriodDates, getCurrentMonthlyPeriodDates, getPreviousMonthlyPeriodDates } from './utils/dateCalculations.js';
import { getMentorshipStartTime, getMentorshipEndTime, calculateDuration, determineServiceType, calculateMentorshipStatus, getStudentName, getServiceName, } from './utils/mentorshipProcessing.js';
...
export async function authMiddleware(req, res, next) {
    ...
    try {
        ...
        req.user4GeeksData = userData; // Guarda los datos del usuario en la request
        next();
    }
    catch (err) {
...
app.post('/api/mentor-nps-test', async (req, res) => {
    try {
        ...
        let allMentorCohortIds = new Set();
        if (COHORTS_DB && COHORTS_DB_FOR_EVALS_SEARCH) {
            try {
                ...
                try {
                    ...
                    taDirectQuery.results?.forEach((page) => allMentorCohortIds.add(page.id));
                    const taRollupQuery = await notion.databases.query({
                        database_id: COHORTS_DB_FOR_EVALS_SEARCH,
                        filter: { property: 'T.A.', rollup: { any: { relation: { contains: originalMentorId } } } },
                        page_size: 100
                    });
                    taRollupQuery.results?.forEach((page) => allMentorCohortIds.add(page.id));
                }
                catch (error) {
                ...
                try {
                    ...
                    teacherDirectQuery.results?.forEach((page) => allMentorCohortIds.add(page.id));
                    ...
                    teacherRollupQuery.results?.forEach((page) => allMentorCohortIds.add(page.id));
                }
                catch (error) {
        ...
        if (allMentorCohortIds.size > 0) {
            try {
                ...
                if (allPages.length > 0) {
                    ...
                    const taIds = taRelation.map((t) => t.id).filter((id) => typeof id === 'string');
                    const normalizedTaIds = taIds.map((id) => id.replace(/-/g, ''));
                    if (normalizedTaIds.includes(normalizedMentorId)) {
                        ...
                        const teacherIds = teacherRelation.map((t) => t.id).filter((id) => typeof id === 'string');
        ...
        if (allPages.length > 0) {
            ...
            allPages.slice(0, 3).forEach((page) => {
                ...
                const evalCohortIds = evalCohortRelation.map((c) => c.id);
        ...
        for (const page of allPages) {
            ...
            const teacherIds = teacherRelation.map((t) => t.id);
            const normalizedTeacherIds = teacherIds.map((id) => id.replace(/-/g, ''));
            ...
            // Si está como Teacher, verificar responsabilidad
                ...
                const responsibility = isMentorResponsibleForEvaluation(evaluationDate, mentorChangeDate, teacherIds, normalizedMentorId);
        ...
        if (skippedEvaluations.length > 0 && skippedEvaluations.length <= 5) {
            ...
            skippedEvaluations.forEach((skip) => {
        ...
        const cohortIdsSet = new Set();
        const cohortEvaluations = new Map();
        ...
        const cohortPages = await Promise.all(allCohortIds.map(async (id) => {
            try {
...
app.post('/api/student-info', authorizeRoles(), async (req, res) => {
    try {
        ...
        const cohortRelation = responseStudent.properties?.Cohort?.relation;
    ...
    try {
        ...
        for (const prop of propertiesArray) {
            ...
            // Propiedad multi-select
                ...
                const existingNotionOptionsMap = new Map();
                ...
                const finalMultiSelectMap = new Map();
                incomingTags.forEach(incomingTag => {
                    let tagName;
                    let tagObjectToSend = null;
...
app.post('/api/create-student-comment', authorizeRoles(), upload.array('attachments', 3), async (req, res) => {
    try {
        ...
        const response = await notionApiFetch('https://api.notion.com/v1/comments', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
app.post('/api/search-student-by-email', authorizeRoles(), async (req, res) => {
    try {
        ...
        let cohort = null;
        // Extraer el ID de la cohorte
        const cohortRelation = student.properties?.Cohort?.relation;
        const cohortId = cohortRelation && cohortRelation.length > 0 ? cohortRelation[0].id : null;
        // Si hay cohorte, obtener sus datos
        if (cohortId) {
            cohort = await notion.pages.retrieve({ page_id: cohortId });
...
app.post('/api/cancel-mentorship', authorizeRoles(), async (req, res) => {
    try {
        const { cancellationDate, cancellationNotes, cancellationReason, mentorName, originalMentorshipDate, studentId, supliedWithOtherStudent, mentorshipType } = req.body;
...
app.post('/api/nps-comments', authorizeMentorDataAccess(), async (req, res) => {
    try {
        ...
        const evaluation = evaluationQuery.results[0];
...
const breathcodeMemberIdCache = new Map();
const breathcodeImpersonationTokenCache = new Map();
const studentNameCache = new Map();
const cancellationMentorOptionsCache = new Map();
const mentorNpsResponseCache = new Map();
const mentorMentorshipsResponseCache = new Map();
const studentCommentsCache = new Map();
const npsCommentsCache = new Map();
const notionUserCache = new Map();
const pendingBreathcodeMemberIdLookups = new Map();
const pendingBreathcodeTokenLookups = new Map();
const pendingStudentNameLookups = new Map();
const pendingNotionUserLookups = new Map();
...
function setCachedValue(cache, key, value, ttlMs = DEFAULT_CACHE_TTL_MS) {
    cache.set(key, {
        value,
...
async function getCachedNotionUser(userId) {
...
function computeNps(scores) {
    if (!scores.length)
        return { nps: 0, avg: 0, promoters: 0, passives: 0, detractors: 0, count: 0 };
...
function validateMentorChangeData(teacherIds, mentorChangeDate, evaluationDate) {
    const warnings = [];
...
function isMentorResponsibleForEvaluation(evaluationDate, mentorChangeDate, teacherIds, currentMentorId) {
...
async function resolveMentorIdFromReq(req) {
    const email = req.user4GeeksData?.email;
    if (!email) {
    ...
    try {
        ...
        console.log('✅ [resolveMentorIdFromReq] Mentor encontrado:', {
            mentorId,
            email,
            mentorName: result.results[0].properties?.Name?.title?.[0]?.plain_text || 'Sin nombre'
        });
        return mentorId;
    }
    catch (error) {
...
async function detectMentorCurrentRoleByNotionId(mentorId) {
    ...
    const hasAssistantAssignments = [taDirect, taRollup].some((result) => result.status === 'fulfilled' && (result.value.results?.length || 0) > 0);
    const hasTeacherAssignments = [teacherDirect, teacherRollup].some((result) => result.status === 'fulfilled' && (result.value.results?.length || 0) > 0);
    if (hasTeacherAssignments)
        return 'teacher';
    if (hasAssistantAssignments)
        return 'assistant';
    return null;
}
...
async function fetchJsonWithToken(url, token, options = {}) {
...
function extractEmailFromMember(member) {
...
async function resolveBreathcodeMemberIdByEmail(operatorToken, email) {
    ...
    const lookupPromise = (async () => {
        ...
        for (const academyId of ALLOWED_ACADEMY_IDS) {
            try {
                const payload = await fetchJsonWithToken(`${apiUrl}/auth/academy/${academyId}/member/${userId}`, operatorToken, { academy: academyId });
...
async function resolveBreathcodeMentorNameVariantsByEmail(operatorToken, email) {
    const apiUrl = process.env.BREATHCODE_API_URL;
    if (!apiUrl) {
    ...
    for (const academyId of ALLOWED_ACADEMY_IDS) {
        try {
            const memberPayload = await fetchJsonWithToken(`${apiUrl}/auth/academy/${academyId}/member/${userId}`, operatorToken, { academy: academyId });
            return getMentorNameVariants(`${memberPayload?.first_name || ''} ${memberPayload?.last_name || ''}`, `${memberPayload?.user?.first_name || ''} ${memberPayload?.user?.last_name || ''}`, fallbackName, email);
        }
        catch (error) {
            continue;
        }
...
async function fetchImpersonatedBreathcodeToken(operatorToken, email, memberIdOverride) {
    const apiUrl = process.env.BREATHCODE_API_URL;
    if (!apiUrl) {
        throw new Error('BREATHCODE_API_URL no está configurado en las variables de entorno');
    }
    ...
    const lookupPromise = (async () => {
        ...
        for (const academyId of academyCandidates) {
            try {
                const payload = await fetchJsonWithToken(tokenEndpoint, operatorToken, { method: 'POST', academy: academyId });
                const impersonatedToken = payload?.key || payload?.token || null;
                if (impersonatedToken) {
                    return setCachedValue(breathcodeImpersonationTokenCache, cleanEmail, String(impersonatedToken), IMPERSONATION_TOKEN_CACHE_TTL_MS);
                }
            }
            catch (error) {
                lastError = error;
        ...
        try {
            const payload = await fetchJsonWithToken(tokenEndpoint, operatorToken, { method: 'POST' });
            const impersonatedToken = payload?.key || payload?.token || null;
            if (impersonatedToken) {
                return setCachedValue(breathcodeImpersonationTokenCache, cleanEmail, String(impersonatedToken), IMPERSONATION_TOKEN_CACHE_TTL_MS);
            }
...
function getRequestImpersonationToken(req) {
...
app.post('/api/mentor-nps', authorizeMentorDataAccess(), async (req, res) => {
    try {
        const { mentorId: mentorIdFromBody, startDate, endDate, includePast = true, summaryOnly = false, } = req.body;
        const requestedMentor = await resolveRequestedMentor(req);
        ...
        // Normalizar UUID removiendo guiones para comparaciones en código
        const normalizedMentorId = mentorId.replace(/-/g, '');
        const mentorNpsCacheKey = buildCacheKey('mentor-nps', requestedMentor.email, originalMentorId, startDate, endDate, includePast, summaryOnly);
        ...
        const fallbackRole = hasAllowedAcademyRole(userRoles, ['teacher'])
            ...
            : hasAllowedAcademyRole(userRoles, ['assistant'])
                ? 'assistant'
                : null;
        let effectiveRole = requestedMentor.effectiveRole === 'teacher' || requestedMentor.effectiveRole === 'assistant'
        ...
        let allMentorCohortIdsFromSearch = new Set();
        // Guardar las páginas de cohortes para evitar consultas posteriores
        let allMentorCohortPagesCache = new Map();
        try {
            ...
            const allMentorCohortIds = new Set();
            const allMentorCohortPages = new Map();
            try {
            ...
            if (foundCohortIdsInSearch) {
                ...
                const batchResultsList = await mapWithConcurrency(cohortBatches, 3, async (batch) => {
                    const cohortFilter = {
                        or: batch.map(cohortId => ({
                            property: 'Cohorts',
                            relation: { contains: cohortId }
            ...
            if (allPages.length === 0 && !foundCohortIdsInSearch) {
                const fallbackPages = new Set();
                if (effectiveRole === 'assistant') {
                    // Intentar T.A. directo
                    try {
                        const taDirectFilter = { property: 'T.A.', relation: { contains: originalMentorId } };
                        if (startDate || endDate) {
                            const andFilters = [taDirectFilter];
                            if (startDate)
                                andFilters.push({ property: 'NPS ID', title: { starts_with: startDate.substring(0, 7) } });
                            if (endDate)
                                andFilters.push({ property: 'NPS ID', title: { starts_with: endDate.substring(0, 7) } });
                            const results = await notionQueryAll(NPS_DB, { and: andFilters });
                            results.forEach((page) => fallbackPages.add(page.id));
                        }
                        else {
                    ...
                    // Intentar T.A. rollup
                    try {
                        const taRollupFilter = {
                            property: 'T.A.',
                            rollup: { any: { relation: { contains: originalMentorId } } }
                        };
                        if (startDate || endDate) {
                            const andFilters = [taRollupFilter];
                            if (startDate)
                                andFilters.push({ property: 'NPS ID', title: { starts_with: startDate.substring(0, 7) } });
                            if (endDate)
                                andFilters.push({ property: 'NPS ID', title: { starts_with: endDate.substring(0, 7) } });
                    ...
                    try {
                        const teacherDirectFilter = { property: 'Teacher', relation: { contains: originalMentorId } };
                        if (startDate || endDate) {
                            const andFilters = [teacherDirectFilter];
                            if (startDate)
                                andFilters.push({ property: 'NPS ID', title: { starts_with: startDate.substring(0, 7) } });
                            if (endDate)
                                andFilters.push({ property: 'NPS ID', title: { starts_with: endDate.substring(0, 7) } });
                    ...
                    try {
                        const teacherRollupFilter = {
                            property: 'Teacher',
                            rollup: { any: { relation: { contains: originalMentorId } } }
                        };
                        if (startDate || endDate) {
                            const andFilters = [teacherRollupFilter];
                            if (startDate)
                                andFilters.push({ property: 'NPS ID', title: { starts_with: startDate.substring(0, 7) } });
                            if (endDate)
                                andFilters.push({ property: 'NPS ID', title: { starts_with: endDate.substring(0, 7) } });
                ...
                if (fallbackPages.size > 0) {
                    const fallbackRetrievedPages = await mapWithConcurrency(Array.from(fallbackPages), 5, async (pageId) => {
                        try {
                    ...
                    allPages.push(...fallbackRetrievedPages.filter(Boolean));
                }
            }
        }
        catch (error) {
        ...
        for (const page of pages) {
            ...
            const tas = await Promise.all(taRelation.map(async (ta) => {
                try {
                    ...
                    const taName = taPage.properties?.Name?.title?.[0]?.plain_text ||
                        taPage.properties?.Title?.title?.[0]?.plain_text ||
                        'TA sin nombre';
        ...
        const allKnownMentorCohortIds = new Set();
        ...
        if (COHORTS_DB_FOR_INFO) {
            try {
                ...
                if (missingCohortIds.length > 0) {
                    ...
                    const missingCohortPages = await Promise.all(missingCohortIds.map(async (id) => {
                        try {
        ...
        const cohortPagesFetched = cohortPagesToFetch.length > 0 ? await Promise.all(cohortPagesToFetch.map(async (id) => {
            try {
                const page = await notion.pages.retrieve({ page_id: id });
                allMentorCohortPagesCache.set(id, page);
                return page;
        ...
        for (const cohortPage of cohortPages) {
            ...
            const statusProp1 = cohortPage.properties?.['Cohort Status ']?.select?.name;
            const statusProp2 = cohortPage.properties?.['Cohort Status']?.select?.name;
            const statusProp3 = cohortPage.properties?.Status?.select?.name;
            const status = statusProp1 || statusProp2 || statusProp3 || '';
            const cohortName = cohortPage.properties?.Cohort?.title?.[0]?.plain_text ||
                cohortPage.properties?.Title?.title?.[0]?.plain_text ||
                'Cohorte sin nombre';
            ...
            tables: {
                ...
                recentEvaluations: Array.from(byCohort.values())
                    ...
                    const cohortName = cohortPage ?
                        cohortPage.properties?.Cohort?.title?.[0]?.plain_text ||
                            cohortPage.properties?.Name?.title?.[0]?.plain_text ||
                            cohortPage.properties?.Title?.title?.[0]?.plain_text ||
                            'Cohorte' : 'Cohorte';
                    return {
                        npsId: item.npsId,
...
app.post('/api/mentors/test-assistant', async (req, res) => {
    try {
        ...
        const mentor = mentorQuery.results[0];
        ...
        for (const test of filterTests) {
            try {
                ...
                results.push({
                    ...
                    success: true,
                    error: null,
                    sampleResults: queryResult.results?.slice(0, 3).map((page) => ({
                        id: page.id,
        ...
        try {
            ...
            databaseInfo = {
                id: dbInfo.id,
                title: dbInfo.title?.[0]?.plain_text || 'Sin título',
                properties: Object.keys(dbInfo.properties || {}),
                taPropertyExists: !!dbInfo.properties?.['T.A.'],
                taPropertyType: dbInfo.properties?.['T.A.']?.type || 'No encontrado',
                teacherPropertyExists: !!dbInfo.properties?.['Teacher'],
                teacherPropertyType: dbInfo.properties?.['Teacher']?.type || 'No encontrado'
            };
        }
        catch (error) {
        ...
        try {
            ...
            allEvaluations = allQuery.results?.slice(0, 10).map((page) => ({
                id: page.id,
                npsId: page.properties?.['NPS ID']?.title?.[0]?.plain_text || 'Sin ID',
                taRelation: page.properties?.['T.A.']?.relation || page.properties?.['T.A.']?.rollup || 'Sin relación',
                teacherRelation: page.properties?.['Teacher']?.relation || page.properties?.['Teacher']?.rollup || 'Sin relación',
        ...
        if (allEvaluations.length > 0) {
            const hasReferences = allEvaluations.some((evaluation) => evaluation.hasOurMentorId);
            if (hasReferences) {
        ...
        for (const test of filterTests) {
            try {
                ...
                results.push({
                    ...
                    success: true,
                    error: null,
                    sampleResults: queryResult.results?.slice(0, 2).map((page) => ({
                        id: page.id,
                        npsId: page.properties?.['NPS ID']?.title?.[0]?.plain_text || 'Sin ID',
                        taRelation: page.properties?.['T.A.']?.relation || page.properties?.['T.A.']?.rollup || 'Sin relación'
                    })) || []
...
app.get('/api/mentors/mentor-id-info', authorizeTeachersOrAssistants(), async (req, res) => {
    try {
        ...
        try {
            ...
            mentorInfo = {
                id: mentorPage.id,
                name: mentorPage.properties?.Name?.title?.[0]?.plain_text || 'Sin nombre',
                email: mentorPage.properties?.Correo?.email || 'Sin email',
                createdTime: mentorPage.created_time,
                lastEditedTime: mentorPage.last_edited_time
            };
        }
        catch (err) {
            error = {
...
app.post('/api/mentors/debug', authorizeTeachersOrAssistants(), async (req, res) => {
    try {
        ...
        const userInfo = {
            email: email,
            firstName: req.user4GeeksData?.first_name,
            lastName: req.user4GeeksData?.last_name,
            username: req.user4GeeksData?.username,
            roles: req.user4GeeksData?.roles || []
        };
        ...
        const diagnosis = {
            ...
            mentorInfo: mentorResult?.results?.length ? {
                ...
                name: mentorResult.results[0].properties?.Name?.title?.[0]?.plain_text || 'Sin nombre',
                email: mentorResult.results[0].properties?.Correo?.email || 'Sin email'
            } : null,
            recommendations: []
        };
        // Generar recomendaciones
...
app.get('/api/mentors/me', authorizeTeachersOrAssistants(), async (req, res) => {
    try {
        ...
        const name = page.properties?.Name?.title?.[0]?.plain_text ||
            page.properties?.Title?.title?.[0]?.plain_text ||
            [req.user4GeeksData?.first_name, req.user4GeeksData?.last_name]
                .filter(Boolean)
                .join(' ') ||
            req.user4GeeksData?.username;
 ...
 */
async function getStudentNameFromNotion(studentId) {
    ...
    const lookupPromise = (async () => {
        try {
            ...
            const properties = studentPage.properties;
 ...
 */
async function getStudentNamesBatch(studentIds) {
 ...
 */
function mapMentorshipTypeFromNotion(notionType) {
    if (!notionType)
        return 'Mentoría';
...
function getMentorNameVariants(...mentorNames) {
...
async function getCancellationMentorOptions(databaseId) {
    const cachedOptions = getCachedValue(cancellationMentorOptionsCache, databaseId);
    if (cachedOptions) {
        return cachedOptions;
    }
    const database = await notion.databases.retrieve({ database_id: databaseId });
    const options = (database.properties?.['Mentor/a']?.select?.options || [])
 ...
 */
async function fetchCancellationsFromNotion(mentorNames, currentPeriod, previousPeriod) {
 ...
 */
async function findStudentIdByName(studentName) {
    try {
 ...
 */
async function findExistingCancellation(mentorshipDate, studentId) {
    try {
        const CANCELLATIONS_DB = process.env.NOTION_CANCELLATIONS_DATABASE_ID || '';
        if (!CANCELLATIONS_DB) {
 ...
 */
function mapServiceToNotionType(service) {
 ...
 */
async function updateCancellationReviewRequest(pageId) {
    try {
 ...
 */
async function createCancellationForReview(mentorshipData, mentorName) {
    try {
        const CANCELLATIONS_DB = process.env.NOTION_CANCELLATIONS_DATABASE_ID || '';
        if (!CANCELLATIONS_DB) {
            throw new Error('NOTION_CANCELLATIONS_DATABASE_ID no está configurado');
        ...
        const serviceType = mapServiceToNotionType(mentorshipData.service);
...
app.get('/api/mentor/my-mentorships', authMiddleware, authorizeMentorDataAccess(), async (req, res) => {
    try {
        ...
        const periodType = req.query.periodType || 'academic';
        ...
        const mentorshipsCacheKey = buildCacheKey('mentor-mentorships', requestedMentor.email, requestedMentor.id, periodType, summaryOnly);
        ...
        if (requestedMentor.isImpersonated) {
            try {
                token = providedImpersonationToken ||
                    await fetchImpersonatedBreathcodeToken(operatorToken, requestedMentor.email, requestedMentor.memberId || null);
            }
            catch (error) {
        ...
        const cancellationsPromise = fetchCancellationsFromNotion(await mentorNameVariantsPromise, currentPeriod, previousPeriod);
        const fetchMentorshipSessionsWithRetry = async () => {
            try {
                ...
                const shouldRefreshImpersonationToken = requestedMentor.isImpersonated &&
                ...
                token = await fetchImpersonatedBreathcodeToken(operatorToken, requestedMentor.email, requestedMentor.memberId || null);
        ...
        setCachedValue(mentorMentorshipsResponseCache, mentorshipsCacheKey, responsePayload, MENTOR_RESPONSE_CACHE_TTL_MS);
        res.status(200).json(responsePayload);
    }
    catch (error) {
...
app.post('/api/mentor/request-review', authMiddleware, async (req, res) => {
    try {
        const { cancellationId, mentorshipId, student, studentId, service, startTime, endTime, duration, status } = req.body;
        // Validaciones
        if (!mentorshipId) {
        ...
        if (cancellationId) {
            ...
            try {
                ...
                const properties = cancellationPage.properties || {};
                const studentRelation = properties['Estudiante']?.relation;
                if (studentRelation && Array.isArray(studentRelation) && studentRelation.length > 0) {
